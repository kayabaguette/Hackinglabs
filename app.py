...
from models import db, Workspace, Note, TerminalLog, ChecklistItem, Snippet
import datetime
...
def get_interfaces():
    interfaces = []
    try:
        # Using 'ip -o -4 addr show' to get IPv4 addresses in one line per interface
        output = subprocess.check_output(['ip', '-o', '-4', 'addr', 'show']).decode('utf-8')
        for line in output.splitlines():
            # Example: 2: eth0    inet 172.17.0.2/16 brd ...
            parts = line.split()
            if len(parts) >= 4:
                name = parts[1]
                ip_cidr = parts[3]
...
def create_workspace():
    ...
    default_items = ["Reconnaissance / OSINT", "Port Scanning (Nmap)", "Service Enumeration", "Vulnerability Analysis", "Exploit Research", "Initial Access (User)", "Privilege Escalation (Root)"]
    for item_name in default_items:
        db.session.add(ChecklistItem(workspace_id=workspace.id, name=item_name))
    db.session.commit()
...
# --- Checklist API ---
@app.route('/api/workspaces/<int:workspace_id>/checklist', methods=['GET'])
def get_checklist(workspace_id):
    items = ChecklistItem.query.filter_by(workspace_id=workspace_id).order_by(ChecklistItem.id).all()
    return jsonify([{'id': i.id, 'name': i.name, 'completed': i.completed} for i in items])

@app.route('/api/workspaces/<int:workspace_id>/checklist', methods=['POST'])
def create_checklist_item(workspace_id):
    data = request.get_json()
    if not data or 'name' not in data:
        return jsonify({'error': 'Name is required'}), 400

    item = ChecklistItem(workspace_id=workspace_id, name=data['name'])
    db.session.add(item)
    db.session.commit()
    return jsonify({'id': item.id, 'name': item.name, 'completed': item.completed})
...
def get_snippets():
    snippets = Snippet.query.order_by(Snippet.name).all()
    return jsonify([{'id': s.id, 'name': s.name, 'command': s.command} for s in snippets])
...
def create_snippet():
    ...
    snippet = Snippet(name=data['name'], command=data['command'])
    db.session.add(snippet)
    db.session.commit()
    return jsonify({'id': snippet.id, 'name': snippet.name, 'command': snippet.command})
